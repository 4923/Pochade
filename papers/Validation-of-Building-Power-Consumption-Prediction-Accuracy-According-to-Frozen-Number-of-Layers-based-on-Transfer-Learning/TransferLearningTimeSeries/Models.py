...
def FitEvaluate(time_series, params):
    ...
    new_model, predictions, history = FitForecast(
        X_train,
        y_train,
        X_test,
        n_ahead,
...
def TransferLearning(time_series, params, model):
    ...
    model_noTransfer, predictions_noTransfer, _ = FitForecast(
        X_train,
        y_train,
        X_test,
        n_ahead,
    ...
    model_withTransfer, predictions_withTransfer, _ = FitForecast(
        X_train,
        y_train,
        X_test,
        n_ahead,
    ...
    mse_withTransfer = mean_squared_error(
        y_true=y_test, y_pred=predictions_withTransfer
    )
...
def generalTuning(series1, series2, params):
    ...
    X_train_target, y_train_target, X_test_target = getInputOutput(
        series_target, input_size
    )
    ...
    model_general, predictions_pre_tuned, hist = FitForecast(
        X_train_general,
        y_train_general,
        X_test_target,
        n_ahead,
    ...
    model_target, predictions_target, hist2 = FitForecast(
        X_train_target,
        y_train_target,
        X_test_target,
        n_ahead,
    ...
    mse_pre_tuned = mean_squared_error(
        y_true=y_test_target, y_pred=predictions_pre_tuned
    )
    ...
    view_predictions(
        [series_general, series_target],
        predictions_pre_tuned,
        y_test_target,
        title="Pre-tuned",
    )
    view_predictions(
        [series_general, series_target], predictions_tuned, y_test_target, title="Tuned"
    )
    view_predictions(
        series_target, predictions_target, y_test_target, title="Target only"
    )
